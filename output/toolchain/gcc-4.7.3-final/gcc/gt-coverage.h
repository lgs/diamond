...
gt_pch_nx_coverage_data (void *x_p)
{
  struct coverage_data * x = (struct coverage_data *)x_p;
  struct coverage_data * xlimit = x;
  while (gt_pch_note_object (xlimit, xlimit, gt_pch_p_13coverage_data, gt_ggc_e_13coverage_data))
   xlimit = ((*xlimit).next);
  while (x != xlimit)
    {
...
{
  struct coverage_data * const x ATTRIBUTE_UNUSED = (struct coverage_data *)x_p;
  if ((void *)(x) == this_obj)
    op (&((*x).next), cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).fn_decl), cookie);
